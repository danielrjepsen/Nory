...
export function ThemeEditorPreview({ theme }: ThemeEditorPreviewProps) {
  ...
  const colorPalette = [
    { name: t('preview.colors.primary'), color: theme.primaryColor },
    { name: t('preview.colors.secondary'), color: theme.secondaryColor },
    { name: t('preview.colors.accent'), color: theme.accentColor },
    { name: t('preview.colors.bg1'), color: theme.backgroundColor1 },
    { name: t('preview.colors.bg2'), color: theme.backgroundColor2 },
  ];

  return (
    <div>
      <h3 className="text-lg font-semibold text-gray-900 mb-5">{t('preview.livePreview')}</h3>
      ...
      <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
        <div className="max-w-[300px] w-full bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-200 mx-auto">
          ...
          >
            ...
            <h1 className="text-xl font-bold mb-1 m-0">{t('preview.eventName')}</h1>
            <p className="text-xs opacity-60 m-0">{t('preview.eventDescription')}</p>
          </div>
      ...
      <div className="mt-5 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <h4 className="text-sm font-medium text-gray-700 mb-3 m-0">{t('preview.colorPalette')}</h4>
